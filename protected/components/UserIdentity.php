...
class UserIdentity extends CUserIdentity
{
    ...
    */
    ...
    public function authenticate()
    {
        ...
        $admin = Admins::model()->find('LOWER(login)=?', array(strtolower($this->username)));
        if (($admin === null) || (md5($this->password) !== $admin->password)) {
            $this->errorCode = self::ERROR_USERNAME_INVALID;
        } else {
            ...
            // или что-либо ещё.
            $this->setState('alias', $admin->alias);
            $this->setState('roles', $admin->role);
//            $this->setState('count_log',AuditTrail::model()->count());
